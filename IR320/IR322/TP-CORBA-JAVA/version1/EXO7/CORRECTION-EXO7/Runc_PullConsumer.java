...
public class Runc_PullConsumer
	{
	public static void main(String args[]) throws IOException
		...
		BOA boa = ((com.ooc.CORBA.ORB)orb).BOA_init(args, props);
                ...
                if(args.length!=1)
                        {
                        System.err.println("utilisation : Runc_PullConsumer nom-du-canal  ");
                        System.exit(1);
                        }
		...
		//
		PullConsumer_Impl consumer = new PullConsumer_Impl();
		...
		PullSupplierHolder supplier = new PullSupplierHolder();

		factory.get_channel(ch, args[1]);
		ch.value.connect_PullConsumer(consumer,
			 supplier);
		...
		while(!consumer.is_disconnected.get())
			{
 			...
 			try {
				org.omg.CORBA.Any any=supplier.value.pull();
				System.out.println("Message recu : " + any.extract_string() );
				...
				{
				System.out.println("PullConsumer disconnected");
				System.exit(0);
				}
